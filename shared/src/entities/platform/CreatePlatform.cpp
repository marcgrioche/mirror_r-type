...
Entity factories::createOneWayPlatform(std::shared_ptr<Registry> registry, float posx, float posy)
{
...
Entity factories::createPlatform(std::shared_ptr<Registry> registry, float posx, float posy)
{
    Entity platform = registry->create_entity();
    registry->emplace<Position>(platform, Position { posx, posy });
    registry->emplace<Velocity>(platform, 0.0f, 0.0f);
...
std::vector<Entity> factories::generateRandomPlatforms(std::shared_ptr<Registry> registry, int quantity)
{
