...
void movementPlatform(std::shared_ptr<Registry> registry, float deltaTime)
{
...
void movementSystem(std::shared_ptr<Registry> registry, float deltaTime, std::optional<Entity> t_onlyEntity)
{
    // store previous positions for entities that need it
    if (t_onlyEntity.has_value()) {
        auto& prevPosEntity = registry->get<PreviousPosition>(*t_onlyEntity);
        auto& posEntity = registry->get<Position>(*t_onlyEntity);
        auto& velEntity = registry->get<Velocity>(*t_onlyEntity);

        prevPosEntity.x = posEntity.x;
        prevPosEntity.y = posEntity.y;
        posEntity.x += velEntity.dx * deltaTime;
        posEntity.y += velEntity.dy * deltaTime;
        return;
    }
