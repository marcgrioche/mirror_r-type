...
\vspace{0.2em}
\textit{Détails complets dans le document COMPARATIVE\_STUDY.md.}
\end{frame}
...
% ──────────────────────────────────────────────────────────────────────────────
\begin{frame}{B2-C09.1 — Architecture}
\footnotesize
Architecture \textbf{ECS + Client/Serveur} :
...
\vspace{0.2em}
\textbf{Modèle réseau :}\\
Serveur autoritaire avec prédiction côté client et réconciliation.
\end{column}
...
\begin{tikzpicture}[
  node distance=0.3cm,
  box/.style={draw, rounded corners, minimum width=2.2cm, minimum height=0.6cm, font=\tiny\bfseries, text=white, align=center},
  layer/.style={draw, rounded corners, minimum width=5.0cm, minimum height=0.5cm, font=\tiny, align=center, fill=rtypelight, text=rtypedark},
  arr/.style={-{Stealth[length=2.5pt]}, thick},
]
  % Composants principaux
  \node[box, fill=rtypeblue] (client) {Client SDL2};
  \node[box, fill=rtypeorange, right=1.2cm of client] (server) {Serveur};
  \node[box, fill=rtypegreen, below=of server] (db) {SQLite3};

  % Flèches de connexion
  \draw[arr, <->] (client.east) -- node[above, font=\tiny] {UDP} (server.west);
  \draw[arr, <->] (server.south) -- (db.north);

  % Couches architecturales (empilées verticalement)
  \node[layer, below=1.2cm of client.south west, anchor=north west, xshift=-0.3cm] (l1) {\textbf{Réseau} — UDP + protocole binaire custom};
  \node[layer, below=0.15cm of l1] (l2) {\textbf{Logique} — GameInstance, systèmes ECS};
  \node[layer, below=0.15cm of l2] (l3) {\textbf{Rendu} — SDL2, sprites, parallax};
  \node[layer, below=0.15cm of l3] (l4) {\textbf{Données} — SQLite3, JSON niveaux};

  % Cadre autour des couches
  \node[draw, dashed, rounded corners, fit=(l1)(l2)(l3)(l4), inner sep=3pt, label={[font=\tiny\bfseries, text=rtypeblue]above left:Couches}] {};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
